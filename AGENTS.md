...
- `{ version: 1, app: { pub }, bsv?, icp?, kda?, proofs, canonicalId }` — **V1, standard.** Purpose-scoped keys; the parent signs with the **app-specific** key `app.pub`.
...
2. **Never read `wallet.*` without `if (me.version === 0)`, nor `bsv/icp/kda/app` without `if (me.version === 1)`.** TypeScript enforces this; respect it.
