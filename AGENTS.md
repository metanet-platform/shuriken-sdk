...
- **Clear the platform's nav bar (don't hardcode its height):** `ninja.layout()?.navBottom ?? 58` → CSS px your top chrome must stay below; `null` means a legacy parent (use the fallback). Subscribe to platform redesigns with `ninja.on('layout', ({ navBottom }) => …)`. To restyle the nav for your app pass `connect({ nav: { bg?, width? (px | 'full'), roundedBottom?, sideMargins? } })` — advisory, sanitized/clamped by the parent, auto-reverted when the user navigates away; the `layout` you receive is measured AFTER your prefs are applied.
- **Localize your app:** `ninja.locale() ?? new URLSearchParams(location.search).get('metanetLang') ?? 'en'` — the protocol field is live (`ninja.on('locale', lng => i18n.changeLanguage(lng))` follows mid-session switches); the `metanetLang` query param is DEPRECATED and only carries the load-time value (keep it solely as the legacy-parent fallback).
